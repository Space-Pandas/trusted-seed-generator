...
fn main() {
    ...
    println!("Running pandas seed generator...");
    let mut retval = sgx_status_t::SGX_SUCCESS;
    let result = unsafe { generate_seed(enclave.geteid(), &mut retval, opt.chain_id, opt.mint_id, sign_type) };
    match result {
        sgx_status_t::SGX_SUCCESS => {
            println!("pandas: ECALL success!");
        }
        _ => {
