...
pub fn create_attestation_report(
    ...
    report_data.d[..4].clone_from_slice(&chain_id.to_le_bytes());
    report_data.d[4..8].clone_from_slice(&mint_id.to_le_bytes());
    report_data.d[8..12].clone_from_slice(&seed.to_le_bytes());
...
#[no_mangle]
pub extern "C" fn generate_seed(
    chain_id: c_uint,
    mint_id: c_uint,
    sign_type: sgx_quote_sign_type_t,
) -> sgx_status_t {
    let mut os_rng = os::SgxRng::new().unwrap();
    let seed = os_rng.next_u32();
    println!("here");
    println!("pandas-info: chain: {}, mint id: {}", chain_id, mint_id);
    println!("pandas-seed: {}", seed);

    let (attn_report, sig, cert) =
        match create_attestation_report(chain_id, mint_id, seed, sign_type) {
            Ok(r) => r,
            Err(e) => {
